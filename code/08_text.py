...
nltk.download("punkt_tab", quiet=True)     # what word_tokenize loads in nltk >= 3.8.2 (Lab 7 downloads the same)
nltk.download("punkt", quiet=True)         # older nltk installs
...
for ax, (name, sub) in zip(axes, [("non_human", weights.head(15)), ("human", weights.tail(15)[::-1])]):
    ...
    ax.set_title("words the model weights towards " + name)
plt.tight_layout()
plt.savefig(OUT / "fig_text_model_words.png", dpi=120, bbox_inches="tight")
plt.show()
...
# Task 5: the model fitted on every labelled row (Task 2b) predicts the unknown profiles; write every row
unknown = ~labelled
prob_unknown = clf.predict_proba(X[unknown.to_numpy()])[:, 1]
predictions = pd.concat([
    pd.DataFrame({"_unit_id": lab_rows["_unit_id"].to_numpy(), "says": says_all,
                  "score": score_all.round(3), "recorded": label[labelled].to_numpy()}),
...
print("text_predictions.csv:", len(predictions), "rows (labelled out-of-fold + unknown)")
